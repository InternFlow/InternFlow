...
console.log(offerId);
  ...
  const handleinformercandidat = async (offerId, candidate) => {
    try {
      const response = await fetch(`http://localhost:5000/Candidacy/notify/${candidate.intern._id}/quiz/${idd}`, 
      {
        method: "POST",
              headers: {
                "Content-Type": "application/json",
...
//ahmed
 ...
 // useEffect(()=>{
  ...
  const toCandidacy = (idP) => {
console.log(idP);
   history.push(`/showApplyOffer?idC=${idP}`);
                        ...
                        <React.Fragment key={index}>
                          <tr>
                            ...
                            <td>{candidate.intern.email}</td>
                                ...
                                <td>
                                <Button color="success" onClick={() => toCandidacy(candidate._id)} >
                                  Consult the Apply
                                </Button> 
                                  </td>                           
                            ...
                            <td>
                              ...
                              {candidate.status === 'approved' && candidate.statusQuiz === 'pending' && (
                               ...
                               {candidate.status === 'approved' && candidate.statusQuiz === 'intermediaire' && (
                                <Button color="primary" onClick={() => handleinformercandidat(offerId, candidate)} >
                                  envoyer les formations
                                </Button>
                              )}
