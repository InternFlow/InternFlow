...
function CourseListItem(props) {
  const { name, description, price, image, trainer, _id } = props.course;
  ...
  if (token) {
    fetch('http://localhost:5000/profile/'+trainer._id, {
      method: 'GET',
      headers: {
  ...
  return (
    ...
    <Card className="mb-1 h-100"   style={{ marginBottom: "180px", alignItems: "center",  margin: "20px" }} >
      ...
      <CardBody> 
        ...
        {trainerData &&(<Link to={"/Profile/"+trainer._id}>
      ...
      <CardFooter>
      {role ==='condidat' &&(<>
        {!Enrolled ? (<Button onClick={enrollStudentInCourse} disabled={(props.course.students.length >= props.course.maxParticipants)} >Enroll</Button>):(<Button onClick={handleUnenroll}>unEnroll</Button>)}</>
        )}
      </CardFooter>
