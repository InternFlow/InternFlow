...
const AddCourseForm = (props) => {
  ...
  const {onCourseAdded, trainer} = props;
  ...
  const createCourse = async (course) => {
    try {
      const response = await fetch('http://localhost:5000/Course/'+trainer._id, {
        method: 'POST',
        headers: {
      ...
      setCourseAdded("Course added successfully")
      onCourseAdded();
      return newCourse;
    } catch (error) {
      console.error(error);
      setCourseAdded("Something went wrong")
    }
  };
  ...
  const handleSubmit = (event) => {
    ...
    // Handle submit logic here, such as calling an API to add the course
  };
...
/*
  ...
  <Card style={{padding:"5%"}}>
    <CardTitle>Add Course To {'"' +trainer.name + '"' + " - " +trainer.email}</CardTitle>
    ...
    <FormGroup>
      ...
      <Button color="primary" onClick={handleSubmit}>Add Course</Button>
      {courseAdded&&(
        <>
        {
          courseAdded === "Course added successfully" ? (<Alert color="success">
            {courseAdded}
          </Alert>):(
            <Alert color="danger">
            {courseAdded}
