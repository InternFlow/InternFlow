...
const  ApplyPage=()=> {
  ...
  const idI = new URLSearchParams(location.search).get('param');

  const idO = new URLSearchParams(location.search).get('param2');
...
React.useEffect(() => {
  ...
  React.useEffect(() => {
    fetch(`http://localhost:5000/Candidacy/showApply/?id=${idI}&idO=${idO}`, {
        method: 'GET',
        headers: {
