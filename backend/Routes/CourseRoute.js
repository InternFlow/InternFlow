...
router.get('/trainer/:trainerId', async (req, res) => {
  try {
    const courses = await Course.find({ trainer: req.params.trainerId });
...
router.get('/trainer/:trainerId', requireAuth, checkRole('formateur'), async (req, res) => {
  try {
...
router.post('/', requireAuth, checkRole('formateur'),async (req, res) => {
  const { name, description, price, enrollmentStartDate, enrollmentEndDate, courseStartDate, courseEndDate, tags, image } = req.body;
  const trainer = req.user._id;
  try {
    const course = new Course({ name, description, price, enrollmentStartDate, enrollmentEndDate, courseStartDate, courseEndDate, tags, image, trainer });
...
router.put('/:id', requireAuth, checkRole('formateur'), async (req, res) => {
  ...
  const { name, description, price, enrollmentStartDate, enrollmentEndDate, courseStartDate, courseEndDate, tags, image } = req.body;
  try {
    const updatedCourse = await Course.findByIdAndUpdate(id, { name, description, price, enrollmentStartDate, enrollmentEndDate, courseStartDate, courseEndDate, tags, image }, { new: true });
    if (updatedCourse) {
...
router.delete('/:id', requireAuth, checkRole('formateur'), async (req, res) => {
  const { id } = req.params;
  try {
