...
router.put('/offer/:offerId/users/:userId/status', requireAuth,checkRole("company"), async (req, res) => {
  try {
    console.log(req.body.status);
      ...
      if(offer.quizzes.length===0)
        {
          return res.status(400).json({ error: `L'offre ${offer.name} est vide. Veuillez le remplir un quiz pour pouvoir valider son statut et envoyer tous les quiz.` });
...
console.log(idC);
  ...
  try {
    ...
    const apply = await Candidacy.findById(idC).populate('intern');
    console.log("aywah");
    console.log(apply);
...
router.get('/getListCandidacyInter/:id', async (req, res) => {
  try {
    ...
    Candidacy.find({ 'intern': req.params.id })
  .populate({
    path: 'offer',
    select: 'title',
    populate: {
...
// Route qui renvoie la prédiction pour un pourcentage donné
router.get('/prediction/:percentage', (req, res) => {
  const pourcentage_test = parseInt(req.params.percentage);
  
  // Exécution du script Python avec le pourcentage comme argument
  const pythonScriptPath = path.join(__dirname, 'python', 'mypython.py');
  console.log("fffffffffff" , pythonScriptPath)
  const python = spawn('python', [pythonScriptPath, pourcentage_test.toString()]);

  // Récupération de la sortie standard du script Python
  python.stdout.on('data', (data) => {
    const prediction = data.toString().trim();
    res.send(`Pourcentage: ${pourcentage_test}% -> Niveau: ${prediction}`);
  });

  // Gestion des erreurs de l'exécution du script Python
...
router.post('/offer/:offerId/quiz/:quizId/submit', requireAuth, checkRole("condidat"), async (req, res) => {
  try {
    ...
    const quizScoreTotal = quizResults.reduce((total, result) => total + result.score, 0);
    const quizScorePercentage = (quizScoreTotal / quiz.scoremax) * 100;
    
    const quizScore = { quizId: quiz._id, name: quiz.name, score, percentage: quizScorePercentage };
    application.quizScores.push(quizScore);
    application.quizResults = quizResults;
    
    // Calcule le pourcentage moyen de tous les quiz
    const quizScores = application.quizScores;
    const totalPercentage = quizScores.reduce((total, quizScore) => total + quizScore.percentage, 0);
    const averagePercentage = totalPercentage / quizScores.length;
    application.percentage_moyenne_quiz = averagePercentage;
    

    
    await application.save();

  // Exécution du script Python avec le pourcentage moyen de tous les quiz en tant qu'argument
console.log("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa   "+    application.percentage_moyenne_quiz.toString())
const pythonScriptPath = path.join(__dirname, 'python', 'mypython.py');
console.log("fffffffffff" , pythonScriptPath)
const python = spawn('python', [pythonScriptPath, application.percentage_moyenne_quiz.toString()]);

// Récupération de la sortie standard du script Python
python.stdout.on('data', async (data) => {
  const prediction = data.toString().trim();
  console.log("bbbbbbbbbbbbb   "+    prediction)
 
  // Modification du statusQuiz selon la prédiction
  let statusQuiz;
  if (prediction === 'debutant') {
    statusQuiz = 'rejected';
  } else if (prediction === 'intermediaire') {
    statusQuiz = 'intermediaire';
  } else {
    statusQuiz = 'approved';
  }

  // Mise à jour de la candidature avec le nouveau statusQuiz
  application.statusQuiz = statusQuiz;

  // Enregistrement de la candidature modifiée dans la base de données
  await application.save();

  // Renvoi de la réponse au client avec les résultats du quiz et le nouveau statusQuiz
});

// Gestion des erreurs lors de l'exécution du script Python
python.stderr.on('data', (data) => {
...
router.post('/notify/:userId/quiz/:offerId', requireAuth, checkRole('company'), async (req, res) => {
  ...
  try {
    ...
    const newNotification = {
      message: `Merci de passer les quiz pour l'offre ${offer.title}. Vous êtes en liste d'attente pour avoir une meilleure chance. Vous pouvez vous inscrire dans ces formations pour augmenter vos chances.`,
      formationlink: 'formation',
      offreid: offerId
    };
    user.notifications.push(newNotification);
    await user.save();
    console.log(`L'utilisateur ${userId} a été notifié de passer les quiz pour l'offre ${offerId}`);
